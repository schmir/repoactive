...
def boxquote(msg: str, title: str = "") -> str:
    ...
    The box opens with ,----[ title ] (or just ,---- when title is empty), each
    line of msg is prefixed with "| ", and the box closes with `----.
    ...
    align under that first line, the closing "]" trails the last of them, and a
    blank "|" line then separates the bracketed title from msg. A trailing
    newline on title is dropped so it does not produce an empty final title
    line.
    """
...
def strip_boxquotes(text: str) -> str:
    """Remove every boxquote block (see boxquote) from text.

    A block runs from a ,---- opening line to its closing `---- line; the
    surrounding text is kept. Blank lines exposed by a removed block are
